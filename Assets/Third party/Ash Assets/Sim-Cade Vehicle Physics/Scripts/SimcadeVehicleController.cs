...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
		...
		{
            _inputActive = _inputManager.InputActive;

            _inputManager.InputActiveChanged += HandleInputActiveChanged;
		}
        ...
        private void Update()
        {
            if (_inputActive && CanDrive && CanAccelerate)
            {
                accelerationInput = Input.GetAxis("Vertical");
                steerInput = Input.GetAxis("Horizontal");
                brakeInput = Input.GetAxis("Jump");
            }
            else if(_inputActive && CanDrive && !CanAccelerate)
            {
                accelerationInput = 0;
                steerInput = Input.GetAxis("Horizontal");
                brakeInput = Input.GetAxis("Jump");
		...
		private void HandleInputActiveChanged(bool newValue)
        {
            _inputActive = newValue;
        }
